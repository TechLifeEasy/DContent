...
contract DeContent {
    ...
    function getPosts() view public returns(Post_data[] memory) {
        require(msg.sender != address(0));
        require(isUser[msg.sender]);
    ...
    function getAllPosts() view public returns(Post_data[] memory) {
        ...
        for(uint i=0; i<user_id.length; i++) {
            Post_data[] memory user_id_posts = getPosts(user_id[i]);
            Post_data[] memory temp = new Post_data[](all_posts.length + user_id_posts.length);
            for(uint i=0; i<all_posts.length; i++) {
                temp[i] = all_posts[i];
            }
            for(uint i=0; i<user_id_posts.length; i++) {
                temp[i] = user_id_posts[all_posts.length + i];
            }
            all_posts = temp;
